...
## 2026-07-23 — Manager-review round shipped: overlay-input UI, emergency resolution, GIPSA LOS; Phase-1 verified

Six things landed today (0A/0B on prod; GIPSA on prod; 0C explainer held; Phase-1 verified). Driven by the 23-Jul manager meeting + his reviewed T1–T28 doc.

**0A — Overlay INPUT controls on the HO estimate form (the #1 gap).** In the meeting the manager tried to build emergency/positive/etc. cases and couldn't find where to enter them (he typed "emergency craniectomy" as the treatment; see 0B). The engine always accepted the `controls.*` overlay fields, but the HO v3 form's `buildRequest` didn't send them and there was no UI. Added `V3Overlays` + `overlayControls()` in `EstimateV3Page.tsx` (maps all 14 overlays — emergency, positive-case, cross-consult, blood, attendant, newborn, chemo, medical-mgmt, daycare, labour, tax, outside-LOS, manual-addons, pharmacy — to the exact engine control contract) and a collapsible **"Add-ons & overlays"** panel (`components/OverlayInputs.tsx`) in the Inputs screen. All optional/off by default; persists in the saved payload; the read-only OverlaySection on the Preview renders the result. **FCs can now author overlay cases in the form** (not just view them). Verified e2e on dev then **promoted to prod** (HO main). Manager-approved via doc Section D ("Looks good").

**0B — "emergency <procedure>" no longer blocks the family match.** Manager wrote "emergency craniectomy" and it failed to resolve — the "emergency" token diluted/blocked the craniectomy match. Fix in `resolve/familyResolve.js familyMatches()`: the AI system prompt now states that admission-context / urgency words (emergency, emergent, elective, planned, scheduled, urgent, stat, routine) describe WHEN care happens, not WHAT the procedure is, and to match the underlying procedure — while still mapping genuine trauma/resuscitation wording to `emergency_trauma_stabilization` (that family EXISTS, so a blunt token-strip was wrong). Verified live: "emergency craniectomy" → `craniotomy_craniectomy_and_brain_tumor_surgery` (high); "elective lscs" → `lscs_caesarean` (reason: "admission context word disregarded"). **On prod** (engine main).

**GIPSA package LOS — ward/ICU split now feeds T10's per-setting ledger.** Manager supplied `gipsa Pkg def Master.xlsx` (223 GIPSA "Operation Packages"), authoritative for **package LOS + ICU/ward allocation ONLY — NOT rates** (rates stay from the current TR290 rows in the Service-All TR matrix). Added migration `004_package_master_ward_icu_stay.sql` (columns `pkg_defined_ward_stay` / `pkg_defined_icu_stay` / `los_source`; additive, no rate column), loaded the split for 220/223 TR290 packages (3 unmatched → hospital: GYN5069/OPT0262/OPT0260), and wired `withPackageLos()` in `packages.service.js` to surface the two columns onto `packageOffer.package`. `outsidePackageLos.js` (T10) already switches to `per_setting_ledger` when a ward/ICU breakdown is present — so GIPSA overstays now use the per-setting ledger (unused ward days can't offset excess ICU days) instead of the total-LOS fallback. Verified: TR290/ORT5510 → ward=3/icu=1; sanity 24/0 + 12/0. **On prod** (engine main). Rates untouched.

**Phase-1 corrections — VERIFIED already-built to the manager's final decisions (no change needed):**
- **Combos (T1 Q4 "use the new rules"):** factor-adjusted headline is live in `flow2.service.js` — highest treatment 100% / 2nd 50% / 3rd+ 25%; **cash never reduces**; **GIPSA always** applies; **Non-GIPSA only same-sitting**; the un-reduced 100% sum is kept as `unadjusted_reference`. OPEN (manager confirm): it currently factors the **whole secondary procedure's gross P50**, not PF-only — the standard insurance rule reduces only the secondary's professional fee. Shared LOS/OT overlap across paths is still not modelled (upper bound).
- **T1 insurance PF:** insurance uses the **final-bill cascade** (surgeon 35% / asst-surgeon 25% / anaesthetist 35% / asst-anaesth 25%); **cash stays LAN** (25/15/25/25). The GIPSA-combo "provisional warning" the spec proposed was never built (no-op).
- **T16 tax:** 5% GST on non-ICU room rent **strictly > ₹5,000/day on the full amount** (exactly ₹5,000 → ₹0); **ICU/CCU/ICCU/NICU/HDU exempt**; matched by service **code**, not ward name; **separate line** (A2); attendant room **off by default** (18%, flagged, no code).
- **T20 pharmacy:** high-value selection rate ladder **user_entered → catalog sale_rate → catalog MRP → historic P50 (flagged)**; UOM carried; `replace_family_baseline` so a selected implant/device replaces its family's baseline share (no double-count); routine pharmacy unchanged.
- **T5 DNB:** four-value metadata per line (`billing_disposition` + `four_value` + `fc_hidden`); the FC estimate shows **only `patient_payable`**; patient-₹0 dispositions are hidden from the FC; UI is covered/non-covered only.

**0C — ">₹1,000 add-on drop" (services.js:73-76) explained, NOT changed.** Confirmed it is a *default-vs-optional* gate, **service items only (not pharmacy)**: a service row seen in 75–90% of comparable cases costing **> ₹1,000** is not auto-included — it goes to the optional list (`isDefaultIncluded`: `p>90 || (p>=75 && amount<=1000)`). Proposed fix (held for the manager): surface that band for confirm/AI-preselect and **adjust the bucket residual** (move within P25–P75, never add the full line on top of the P50) to avoid double-count. Written up in `todo_and_helpers/explainer-1000-drop-rule.md`.

**Open decisions with the manager (sent as `todo_and_helpers/review-23Jul.md`):** (1) the >₹1,000 band flip + residual approach; (2) TR-code chip/no-port fallback research; (3) combos PF-only vs whole-procedure; (4) A1 — whether GIPSA general instruments (OTI0014/0101/0018) actually MOVE the amount to the patient as NME (currently labelled `PATIENT_PAYABLE_NME_GIPSA`, amount not moved); (5) export open-bill service + pharmacy line items (biggest unlock — NME Phase-2, full positive-case cohort, DNB ₹1-share, outside-LOS pharmacy). Plus proposed additive enhancements (search-by-name pickers for manual-addon/pharmacy, note→overlay auto-pre-fill, explicit multi-procedure + combo-math display).

**Decision: no unreviewed number change** — 0A/0B/GIPSA are additive/agreed and shipped; the number-changing items (combos PF-only, >₹1,000 preselect, A1 amount-move, shared-LOS de-dup) are held for the manager's yes/no.

## 2026-07-24 — Combo reduction fixed to PROCEDURE-FEE-ONLY (validated on 613 historical combos); LOS = same-sitting/max

Manager's 23-Jul "review 23rd july" replies validated + actioned. Two engine questions (combos, combo-LOS) checked against the hospital's OWN package-bill history (`fc.package_bill_admissions`, 613 comma-separated multi-package combos), then the combo fix shipped.

**Combo reduction — the real bug fixed (`flow2.service.js` combined block ~L188).** History (613 combos): the hospital writes the discount literally into the line/package name (`"…50% Discount"`), and the discounted line is **exactly 0.500×** the full sibling procedure (354 combos carry 50%, 23 carry 25%). It reduces the **whole procedure/package price** (which bundles surgeon PF — so the manager's "reduce the procedure, PF goes with it" is CONFIRMED for package procedures), **but** each secondary procedure's **implants / pharmacy / room stay at FULL** (a bilateral bills two implant sets, both full), and **separately-billed anaesthesia/assistant PF is NOT reduced** (0 of 416 anaesthesia lines discounted). The engine was factoring each path's **whole cohort gross P50** (which bundles implants/pharmacy/room, ~40% of gross) by 0.5 — **under-quoting comparable-value combos ~16%** (bilateral TKR real **1.78×** single vs the engine's **1.50×**). **Fix:** split each path into `fee` (`numbers.package.package_amount` when a package path, else a `0.60` gross proxy = measured hospital-wide fee share) + `extras`; factor the **fee only** (100/50/25), keep extras at full. New multiplier 1.60–1.70× (toward the real 1.78×). Payer/sitting gating unchanged and re-validated (**Cash 5.8% discounted vs GIPSA 84.9% / Non-GIPSA 89.7%** → Cash-never-reduce is correct). `unadjusted_reference` kept. Singles byte-identical (combined block only runs for >1 path). Deployed dev→main.

**Combo LOS — no code change (validated).** All 613 historical combos are **same-sitting** (290/290 with OT lines = single operative day; **zero** staggered — the manager's "TKR day-1 → THR day-4" example has 0 rows in history). Same-sitting combo LOS ≈ **MAX(individual), not sum** (CAG+PTCA combo=2=max n=320; bilateral TKR=4 vs single 3, sum would be 6 n=131; CABG+MVR=9 vs max 8). The engine does NOT model combo LOS today, so there's nothing to fix — the guidance if ever added: same-sitting → MAX(`pkg_defined_ward/icu_stay`); different-sitting → sum only as an explicit hospital assumption (no evidence). Full evidence: `todo_and_helpers/combo-los-historical-validation.md`.

**Caveat carried:** the `0.60` non-package fee-share proxy is data-derived (median 0.606) but only used when a path has no `package_amount`; for package combos the exact package price is used. Separately-billed-PF-not-reduced differs from the manager's "PF at same %" — true only for PF embedded in a package price. FYI-sent to the manager, not a blocker.

**Manager's other 23-Jul replies (no code):** >₹1,000 "keep current"; TR fallback "keep current"; A1 GIPSA instruments "patient-collectible for now, provisional pending hospital" → engine left as-is; open-bill line data "EXISTS in original project Postgres → export/transfer task"; add-on transport "current way fine".
