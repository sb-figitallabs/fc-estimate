...
/**
 * Stream file → gunzip (zlib stream) → CSV rows, with BOUNDED memory.
 * The gunzip stream is consumed via `for await`, so it stays paused while we
 * process each decompressed chunk; `onDataRow` may return a promise (e.g. a
 * mid-stream INSERT flush) which is awaited before the next row — genuine
 * backpressure all the way down to the file read. Only the rows parsed out of
 * a single decompressed chunk are ever held in memory at once.
 * Validates the header, respects --limit (data rows), returns rows delivered.
 */
async function streamCsvGz(filePath, expectedHeader, onDataRow) {
  let header = null;
  let delivered = 0;
  let stop = false;
  const pending = []; // rows parsed from the current chunk (small, drained per chunk)
  const parser = new CsvParser((row) => pending.push(row));
  ...
  const processPending = async () => {
    while (pending.length) {
      ...
      if (limit != null && delivered >= limit) { stop = true; continue; }
      delivered++;
      const obj = {};
      for (let i = 0; i < header.length; i++) obj[header[i]] = row[i];
      const p = onDataRow(obj);
  ...
  src.on('error', (e) => gunzip.destroy(e)); // surface read errors through the async iterator
  src.pipe(gunzip);
...
async function main() {
  ...
  // transaction. Any parse/insert failure rolls back — tables stay intact.
  let client = null;
  try {
    if (!dryRun) {
    ...
    // Streaming insert buffer: at most one batch (500 tuples) of line rows is
    // ever retained. Dry-run builds each tuple (same toNum validation) but
    ...
    const flushLines = async () => {
      ...
      lineBatch = [];
      await batchInsert(client, 'fc.package_bill_lines', LINE_COLS, batch);
    };
    ...
    for (const f of BILL_FILES) {
      ...
      await streamCsvGz(path.join(dir, f), BILL_HEADER, (r) => {
        ...
        lineRowCount++;
        if (dryRun) return; // tuple validated, then discarded — no retention
        lineBatch.push(tuple);
        if (lineBatch.length >= LINE_BATCH_SIZE) return flushLines(); // awaited by streamCsvGz → backpressure
      });
      fileStats.push({ file: f, rows: n, minDt, maxDt });
      console.log(`[parse] ${f}: ${n} line rows, CREATEDT ${minDt} → ${maxDt}`);
    }
    if (!dryRun) await flushLines(); // final partial batch
    ...
    /* --------------------------- database work ------------------------- */
    // (Non-dry-run: DDL/BEGIN/TRUNCATE already ran before parsing; all bill
    // lines were inserted mid-stream inside the same transaction.)
    ...
    if (dryRun) {
      ...
      console.log(`[db] inserted ${lineRowCount} rows into fc.package_bill_lines`);
    ...
    console.log('\n================ COVERAGE REPORT ================');
    console.log(`Rows loaded: admissions=${admissionRows.length}, lines=${lineRowCount}${dryRun ? ' (dry-run: not written)' : ''}`);
    ...
    // Only remove the S3 download tempdir — NEVER the repo-local data dir.
