...
For OT-consumables reviewed canonical logic, the selected shortlist share is then converted into a percentile band using a piecewise threshold model documented in `17_edge_cases_robotic_implants_emergency_mlc.md`.
...
Reviewed OT-consumables threshold behavior:
- selected expected share `<= 0.30` => use `P25`
- selected expected share `<= 0.50` => use `P50`
- otherwise => use `P75`
...
The reviewed auto-selection logic is threshold-driven rather than freeform. See `14_payer_basis_and_payor_selection_rules.md` for:
- exact thresholds
- fallback thresholds
- confidence labels
