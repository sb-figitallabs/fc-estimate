...
Reviewed default-included rule:
- include automatically when presence rate `> 90%`
- also include automatically when presence rate `>= 75%` and typical amount `<= 1000`
...
Reviewed add-on ranking is not arbitrary.
Optional rows are prioritized by:
- expected contribution
- then presence rate
- then rate strength
- then stable grouping / item identity
...
Reviewed thresholds:
- `auto` grouped residuals: presence rate `> 90%` with positive residual
- `optional` grouped residuals: presence rate from `75%` to `90%` inclusive with positive residual
- some investigation groups can be promoted to `auto` when presence, residual amount, and optional-child signals are strong enough
...
- default include `auto` rows
- default exclude `optional` rows unless product intentionally chooses a different UX default
